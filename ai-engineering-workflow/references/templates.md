...
- 进入项目、开工、下一步诊断或执行任务前，先读取 `PROJECT_CONSTRAINTS.md`；本轮请求与项目约束冲突时必须先确认。
...
## 主动触发
当你问“下一步做什么”“今天做什么”“继续”“帮我看当前状态”时，AI 应主动读取 `PROJECT_CONSTRAINTS.md`（如果存在）、`STATUS.md`、`TASK_SPEC.md` 和本文件，先识别当前工作焦点，再判断当前阶段，并给出围绕焦点的未来 3 到 5 步路线图、推荐顺序、首选方案和备选方案。当你在新窗口说“使用这个 Skill，在这个工程中，分析最新测试结果”时，AI 应主动定位最新报告和当前最优基线，输出指标、问题、badcase、可比性和下一步建议。若项目没有最近巡检记录，或存在未验证改动、待确认标准、缺失报告、文档未同步，AI 应主动提醒你可以做一次巡检。若缺少关键信息，AI 应一次最多问 5 个问题，并给示例答案。
...
示例：
硬约束：生产模型/API 服务只能通过 `src/config/model_config.py` 中的硬编码开关切换。
默认禁止：不得新增环境变量覆盖生产服务选择。
冲突处理：如果本轮请求要求临时绕过该约束，必须先说明当前约束并等待确认。
```
